...
possiblePins = range(1000, 10000)  # Range of possible PINs (1000 to 9999)
for possiblePin in possiblePins:
    for R in range(256):  # Iterate over all possible values of R (0 to 255)
        # Encrypt the possiblePin with the current value of R and check if it matches C
        if(RSA_OAEP_Enc(possiblePin, e, N, R) == C):
            pin = possiblePin  # Correct PIN found
            rGiven = R
            break

print("Pin is", pin, "R is ", rGiven)  # Output the found PIN and R
